...
import { ciplCounterService } from '../../core/cipl-counter';
...
export const PrintStation: React.FC<PrintStationProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    const calculateScale = () => {
      if (previewContainerRef.current && activeTemplate) {
        ...
        const padding = 40; // Reduced padding for narrower preview area
        ...
        // Increased min scale to 1.2 for better visibility of small labels
        // This matches printer preview size better
        const calculatedScale = Math.min(scaleX, scaleY);
        setScale(Math.max(calculatedScale, 1.2)); // Min 1.2x, no max limit
      }
    };
    calculateScale();
  ...
  const handlePrint = async () => {
    ...
    // CRITICAL: Wait for all fonts to load before printing
    await document.fonts.ready;

    // Additional verification: Check if barcode fonts are loaded
    const barcodeFontsLoaded = [
      document.fonts.check("12px '3OF9'"),
      document.fonts.check("12px 'LibreBarcode39'"),
      document.fonts.check("12px 'FRE3OF9X'"),
      document.fonts.check("12px 'EanBwr'")
    ];

    // If fonts not loaded, try to load them (but don't show error - this is normal)
    if (!barcodeFontsLoaded.every(loaded => loaded)) {
      try {
        await Promise.all([
          document.fonts.load("12px '3OF9'"),
          document.fonts.load("12px 'LibreBarcode39'"),
          document.fonts.load("12px 'FRE3OF9X'"),
          document.fonts.load("12px 'EanBwr'")
        ]);
      } catch (err) {
        // Silently continue - fonts might already be loaded or not needed
        console.warn("Some fonts could not be loaded:", err);
      }
    }

    // Check if template uses CIPL auto-increment
    const hasCIPL = activeTemplate.elements.some(e => e.bindingKey === '__CIPL_AUTO__');

    // Increment CIPL counter if template uses it
    if (hasCIPL) {
      try {
        await ciplCounterService.getNextNumber();
        // Note: The actual CIPL number will be resolved in getPreviewDisplayValue
      } catch (error) {
        console.error('Failed to increment CIPL:', error);
        addToast('Warning: CIPL number may not have incremented', 'error');
      }
    }

    // Set descriptive filename for PDF
    const originalTitle = document.title;
    document.title = `Label_${selectedProduct.code}_${new Date().toISOString().split('T')[0]}`;
  ...
  const getPreviewDisplayValue = useCallback((element: LabelElementData): string => {
    if (element.isDynamic && element.bindingKey) {
      // Handle CIPL auto-increment
      if (element.bindingKey === '__CIPL_AUTO__') {
        // For preview, show current number from localStorage
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* LEFT PANEL: Input Form (60% width with inline style) */}
        <div
          style={{ width: '60%', minWidth: '350px' }}
          className="p-6 overflow-y-auto flex flex-col gap-6 bg-white border-r-2 border-slate-200 shadow-sm"
        >
          ...
          <div className="space-y-2 relative" ref={dropdownRef}>
            ...
            <div className="relative group">
              ...
              <input
                ...
                className={`w-full pl-12 pr-10 py-4 text-base bg-slate-50 border-2 rounded-xl outline-none transition-all placeholder:text-slate-400 font-medium ${selectedProduct ? 'border-indigo-100 bg-indigo-50/30 text-slate-500 cursor-not-allowed' : 'border-slate-200 focus:border-indigo-500 focus:bg-white focus:shadow-md'}`}
          ...
          {selectedProduct && (
            <div className="space-y-6 animate-in slide-in-from-left-4 duration-300">
              <div className="bg-white border-l-4 border-indigo-500 pl-4 py-2">
                ...
                <div className="text-base font-bold text-slate-800 leading-tight mt-1">{selectedProduct.name}</div>
              ...
              <div className="space-y-4">
                ...
                {requiredFields.plan && (
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Plan / Batch</label>
                    <div className="relative">
                ...
                {requiredFields.remarks && (
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Keterangan (Remarks)</label>
